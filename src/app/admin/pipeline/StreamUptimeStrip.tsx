...
import { fetchStreamUptime, type StreamUptimeResponse } from "./actions";
  ...
  return (
    <div className="rounded-lg border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] p-4 mb-4">
      ...
      {gapSegments.length > 0 ? (
        <div className="mt-3 flex flex-col gap-1">
          {gapSegments.slice(-8).map((g, i) => (
            <div
              key={i}
              className="text-[11px] tabular-nums flex items-center gap-3 text-[var(--color-text-soft)]"
            >
      ...
      <div className="mt-3 text-[10px] text-[var(--color-text-muted)]">
        ...
        Use <code>jobs/backfill_capper_tweets.py --start &lt;gap_start&gt; --end &lt;gap_end&gt;</code> to recover only that window.
      </div>
    </div>
  );
}
