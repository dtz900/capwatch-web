...
// fingerprint (?date=today|tomorrow&d=YYYY-MM-DD&p=picks&s=sharps&g=graded)
// so X's CDN sees a fresh URL whenever the slate changes intra-day or the
// season leaderboard moves. `date` is the only param that affects rendering;
// the rest exist purely as cache-busting keys.
//
