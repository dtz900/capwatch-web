...
export function TofBoard({ rows, me, minPlays }: { rows: TofBoardRow[]; me: TofBoardMe | null; minPlays: number }) {
    ...
    <div className="rounded-xl border border-[rgba(255,255,255,0.09)] bg-[rgba(9,11,12,0.72)] shadow-[0_8px_24px_rgba(0,0,0,0.35)] px-4 py-4">
        ...
        <div className="mt-2 flex flex-col">
          {rows.map((r, i) => (
            <div key={r.username} className="flex items-center gap-3 border-b border-[rgba(255,255,255,0.05)] py-2 text-[12px]">
              ...
              <BoardAvatar url={r.avatar_url} name={r.username} />
              {r.capper_handle ? (
                <Link href={`/cappers/${r.capper_handle}`} className="min-w-0 flex-grow truncate font-bold hover:underline">{r.username}</Link>
              ) : (
      ...
      {me && !meOnBoard && (
        <div className="mt-2 flex items-center gap-3 rounded-lg border border-[rgba(25,245,124,0.2)] bg-[rgba(25,245,124,0.06)] px-2.5 py-2 text-[12px]">
          ...
          <BoardAvatar url={me.avatar_url} name={me.username} />
