 ...
 * brand asset. Always a solid fill at full opacity: the line-art reads
 * crisply from ~26px up but dies as an outline or dimmed. State is a color
 * shift via currentColor: hosts render it soft off-white at rest and brand
 * green once tailing. */
const CROWN_PATH =
...
export function TailCrown({
  ...
  /** Kept for call-site compatibility; state is expressed by the host's
   * text color, not by opacity. */
  filled?: boolean;
  size?: number;
  className?: string;
}) {
  return (
    ...
    >
      <path d={CROWN_PATH} fillRule="evenodd" fill="currentColor" />
    </svg>
  );
}
